  ...
  class Angles
    ...
    def initialize(g_conf, edgeno)
      ...
      (1..@contract[0]).each do |i|
        ...
        Const.assert (edgeno[u][v] >= 1), true, '***  ERROR: CONTRACT CONTAINS NON-EDGE  ***'
    ...
    def angles_sub2(g_conf, edgeno)
      (1..g_conf[0 + 1][0]).each do |v|
        (1..g_conf[v + 2][0 + 1]).each do |h|
          ...
          Const.assert (@contract[a].zero? || @contract[b].zero?), true, str
    ...
    def angles_sub3(g_conf)
      ...
      Const.assert (1 == 2), true, '***  ERROR: CONTRACT HAS NO TRIAD  ***'
      false # ここには来ない
    end
  end
end
